...
const eventSearchUrl =
  "https://app.ticketmaster.com/discovery/v2/events?classificationName=music&countryCode=US&apikey=M9MAwXQGG4GsWcLELZRygN0xaMztNO5y";
const attractionSearchUrl =
  "https://app.ticketmaster.com/discovery/v2/attractions?classificationName=music&countryCode=US&apikey=M9MAwXQGG4GsWcLELZRygN0xaMztNO5y";
...
async function getEventsByLocation(cityName) {
  ...
  const response = await fetch(
    `${eventSearchUrl}&city=${encodeURIComponent(cityName)}`
  );
...
async function getEventsByDate(startDate, endDate) {
  ...
  const response = await fetch(
    `${eventSearchUrl}&localStartEndDateTime=${startDate},${endDate}`
  );
  const result = await response.json();
  const events = result._embedded.events;
...
async function getEventsByArtist(artist) {
  ...
  let response = await fetch(
    `${attractionSearchUrl}&keyword=${encodeURIComponent(artist)}`
  );
  ...
  try {
    ...
    const eventsData = events.map(
      ({ name, priceRanges, url, images, dates }) => {
        return { name, priceRanges, url, images, date: dates.start.localDate };
      }
    );
    return eventsData;
